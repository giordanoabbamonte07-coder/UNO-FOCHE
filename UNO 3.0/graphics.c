...
// Crea una texture di emergenza se manca il file su disco
Texture2D CreateFallback(void) {
...
void InitGraphics(Graphics *gfx) {
    ...
    // Caricamento iterativo di tutte le immagini delle carte
    for (int c = 0; c < 4; c++) {
        for (int v = 0; v <= 9; v++) {
            char path[128];
...
void DrawGame(Graphics *gfx, Game *g) {
    ...
    // Mano Player con evidenziatore carte giocabili
    ...
    if (g->game_over) {
        ...
        if (g->player.num_carte == 0) DrawText("VITTORIA FOCALIZZATA!", 120, 260, 48, GOLD);
        else DrawText("LA FOCA TI HA BATTUTO...", 140, 260, 40, RED);
    }
    EndDrawing();
}
