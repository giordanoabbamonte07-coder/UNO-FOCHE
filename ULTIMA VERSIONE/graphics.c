...
void ScaricaGrafica(Grafica *gfx) {
    if (gfx->sfondo.id > 0 && gfx->sfondo.id != fallback.id) UnloadTexture(gfx->sfondo);
    if (gfx->foca.id > 0 && gfx->foca.id != fallback.id) UnloadTexture(gfx->foca);
    if (gfx->deltaplano.id > 0 && gfx->deltaplano.id != fallback.id) UnloadTexture(gfx->deltaplano);
    if (gfx->win_img.id > 0 && gfx->win_img.id != fallback.id) UnloadTexture(gfx->win_img);
    if (gfx->defeat_img.id > 0 && gfx->defeat_img.id != fallback.id) UnloadTexture(gfx->defeat_img);
    if (gfx->retro.id > 0 && gfx->retro.id != fallback.id) UnloadTexture(gfx->retro);
    if (gfx->btn_sospendi.id > 0 && gfx->btn_sospendi.id != fallback.id) UnloadTexture(gfx->btn_sospendi);
    for (int c = 0; c < 5; c++)
        for (int t = 0; t < 6; t++)
            for (int v = 0; v < 13; v++)
                if (gfx->carte[c][t][v].id > 0 && gfx->carte[c][t][v].id != fallback.id) UnloadTexture(gfx->carte[c][t][v]);
    if (fallback.id > 0) UnloadTexture(fallback);
}
